...
namespace Project.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        [HttpPost("ApiTesting")]
        public async Task<IActionResult> APITesting(string x)
        {
            if (x is null)
                return BadRequest("WORKED WITH NULL! ");
            else
                return Ok("WORKED WITHOUT NULL! ");
        }
        ...
        [HttpPost("PasswordChecker")]
        public async Task<IActionResult> PasswordChecker(PasswordCherckerDTO request)
        {
            var user = await _context.Users.FirstOrDefaultAsync(M => M.Email == request.CurrentEmail);


            if (user is null || (!BCrypt.Net.BCrypt.Verify(request.PasswordChecker, user.passwordHashed)))
                return BadRequest("The user does not exist or the password is incorrect. ");
        ...
        public async Task<IActionResult> updateUserInformation(UpdateUserInformationDTO request) 
        {
            ...
            //might not need for these checkings, as they are handled in PasswordChecker endpoint. 
            //if(user is null || (!BCrypt.Net.BCrypt.Verify(request.password, user.passwordHashed)))
        ...
        //public async Task<IActionResult> ChangeUserPassword(ForgotPasswordDTO request)
        //{
        //    ForgotPasswordDTO(request.newPassword);
        //}
